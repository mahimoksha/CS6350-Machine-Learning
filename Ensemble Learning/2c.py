...
Original file is located at
    https://colab.research.google.com/drive/1LAkMqgqDjZKTjUA4Skpfc3end55SMn5s
"""
...
for i in range(0,100):
    ...
    max_pred = []
    for t in range(500):
        print("*"*10+"for iteration t = "+str(i)+" trees = "+str(t)+"*"*10)
        sample_data =  train_data.iloc[random.sample(range(train_data.shape[0]),1000)]
#         sample_data = train_data.iloc[random_split_data(train_data,samplesize)]
        new_weights = []
        error_t = 0
        algo = ID3()
        labels = ['yes','no']
...
#         max_depth = 1000000000
        ...
        tree_pred = algo.ID3_Algo(sample_data,sample_data,columns[:-1],labels,split_method,[])
...
with open('singletreeclassifiers_100.pickle', 'wb') as handle:
    pickle.dump(single_tree_predictions, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('multi_tree_predictions_100.pickle', 'wb') as handle:
    pickle.dump(multi_tree_predictions, handle, protocol=pickle.HIGHEST_PROTOCOL)


singletrees = []
with (open("singletreeclassifiers_100.pickle", "rb")) as openfile:
    while True:
        try:
...
multitrees = []
with (open("multi_tree_predictions_100.pickle", "rb")) as openfile:
    while True:
        try:
...
print("Bias and Variance of bagged trees is :",(bias,variance))
print("Expected error for bagged trees is ", bias+variance)
